...
async def on_ready():
    ...
    await bot.bot.change_presence(status=discord.Status.online, activity = discord.Game(name=f'g!helpme for a list of commands! 🥳 🎉 Currently in {len(bot.bot.guilds)} servers! 🎉'))
...
async def giveaway(ctx):
    ...
    for question in giveaway_questions:
        ...
        try:
            message = await bot.bot.wait_for('message', timeout= 30.0, check= check)
    ...
    channel = bot.bot.get_channel(c_id)
    ...
    users.pop(users.index(bot.bot.user))
